...
final public class ContextHandlerLC extends AbstractContextHandler {
	...
	private final Logger											LOGGER							= Logger
	    .getLogger(ContextHandlerLC.class.getName());
	 ...
	 * <li>subject id and object id, for the received request, are retrieved from
	...
	public void tryAccess(Message message) {
		...
		if (!isInitialized() || message == null
		    || !(message instanceof TryAccessMessage)) {
			LOGGER.log(Level.SEVERE, "INVALID tryAccess " + isInitialized() + "\t"
			    + (message instanceof TryAccessMessage));
			return;
		}
		...
		System.out
		    .println("[TIME] tryaccess received at " + System.currentTimeMillis());
		
		TryAccessMessage tryAccess = (TryAccessMessage) message;
		
		PolicyHelper policyHelper = PolicyHelper
		    .buildPolicyHelper(tryAccess.getPolicy());
		
		// eventual scheduling
		
		List<Attribute> attributes = policyHelper
		    .getAttributesForCondition(TRYACCESS_POLICY);
		System.out.println(
		...
		/*if (!tryAccess.getScheduled()) {
			...
			if (!ip.equals(getIp()) && !ip.equals("localhost")
				...
				// String request = makeRequestFull(tryAccess.getRequest(), attributes,
				// STATUS.TRYACCESS, false);
				schedule(tryAccess, tryAccess.getRequest(), ip, STATUS.TRYACCESS);
				System.out.println(
		...
		String requestFull = makeRequestFull(request, attributes, STATUS.TRYACCESS,
		    true);
		...
		PDPResponse pdpEvaluation = (PDPResponse) getPdpInterface()
		    .evaluate(requestFull, policyBuilder.append(policy), STATUS.TRYACCESS);
		...
		System.out.println("[TIME] tryaccess evaluated at "
		    + System.currentTimeMillis() + " " + pdpEvaluation.getResponse());
		
		// if access decision is PERMIT - update SM DB entry
		if (pdpEvaluation.getResponse().equalsIgnoreCase("Permit")) {
			
			/**
			 * If tryAccess was scheduled, then the ip to be stored is the one of the
			 * node that has the PEP attached, otherwise it is the URL of this node
			 */
			insertInSessionManager(sessionId, policy, request, status,
			    ...
			    policyHelper,
			    tryAccess.getScheduled() ? tryAccess.getSource() : getIp());
...
//			    TRY_STATUS);
	 ...
	 * <p>
	 * This is the function where the scheduling is effectively performed. Basing
	 * on the type of message received the appropriate action is executed and then
	 * the appropriate node is called.
	 * </p>
	 * 
	 * @param message
	 ...
	 */
	private void schedule(Message message, String request, String ip,
	    STATUS status) {
		
		switch (status) {
			case TRYACCESS:
				// TryAccessMessage tryAccess = (TryAccessMessage) message;
				TryAccessMessage tryAccess = new Gson()
				    .fromJson(new Gson().toJson(message), TryAccessMessage.class);
				tryAccess.getContent()
				    .setPepUri(getIp() + PEP_ID_SEPARATOR + message.getSource());
				...
				System.out.println("[TIME] tryAccess scheduled to other node "
				    + System.currentTimeMillis());
				getRequestManagerToChInterface().sendMessageToOutside(tryAccess);
				return;
			case STARTACCESS:
				StartAccessMessage startAccessMessage = new Gson()
				    .fromJson(new Gson().toJson(message), StartAccessMessage.class);
				getForwardingQueue().addSwappedMessage(message.getID(), message);
				startAccessMessage.setSource(getIp());
				startAccessMessage.setDestination(ip);
				startAccessMessage.setScheduled();
				System.out.println("[TIME] startAccess scheduled to other node "
				    + System.currentTimeMillis());
				getRequestManagerToChInterface()
				    .sendMessageToOutside(startAccessMessage);
				return;
			case ENDACCESS:
				EndAccessMessage endAccessMessage = new Gson()
				    .fromJson(new Gson().toJson(message), EndAccessMessage.class);
				getForwardingQueue().addSwappedMessage(message.getID(), message);
				endAccessMessage.setSource(getIp());
				endAccessMessage.setDestination(ip);
				endAccessMessage.setScheduled();
				System.out.println("[TIME] endAccess scheduled to other node "
				    + System.currentTimeMillis());
			...
			case REEVALUATION:
				...
				System.out.println("[TIME] reevaluation scheduled to other node "
				    + System.currentTimeMillis());
				getRequestManagerToChInterface()
				    .sendMessageToOutside(reevaluationMessage);
				return;
			default:
				return;
		}
	...
	/**
	 * Retrieves the number of atrtibutes managed by each ip
	 * 
	 * @param attributes
	 *          the list of attributes
	 * @return the map in whci hthe key is the IP address of the node and the
	 *         value is the number of attributes managed by that node
	 */
	private synchronized HashMap<String, Integer> retrieveAttributesIp(
	...
	/**
	 * Attempt to make the request full in order to let the PDP evaluate correctly
	 * the request.
	 * 
	 * @param request
	 ...
	 * @param complete
	 *          states if we need to add all the attributes (if true) or only the
	 *          local ones, if false
	 * @return a String that represents the request itself
	 */
	private synchronized String makeRequestFull(String request,
	    List<Attribute> attributes, STATUS status, boolean complete) {
		try {
			RequestType requestType = JAXBUtility.unmarshalToObject(RequestType.class,
			    request);
			// handles all the cases except startaccess
			if (status == STATUS.TRYACCESS || status == STATUS.ENDACCESS
				...
				if (complete) {
					if (getPipRetrieval() != null && external.size() > 0) {
						System.out.println("[TIME] retrieve external start at "
						    + System.currentTimeMillis());
						getPipRetrieval().retrieve(requestType, external);
						System.out.println("[TIME] retrieve external ends at "
						    + System.currentTimeMillis());
					}
					
					if (getPipRetrieval() == null && external.size() > 0) {
						LOGGER.log(Level.SEVERE,
						    "Policy requires attributes that are not accessible!!");
						return null;
					}
				}
			}
			/*
			 * handles the startaccess case which is different because in this case we
			 * have to perform the subscribe operation to signal to the PIPs to
			 * continuously monitor the attributes
			 */
			if (status == STATUS.STARTACCESS) {
				...
				if (complete) {
					if (getPipRetrieval() != null && external.size() > 0) {
						System.out.println("[TIME] Subscribe external starts at "
						    + System.currentTimeMillis());
						getPipRetrieval().subscribe(requestType, external);
						System.out.println("[TIME] subscribe external ends at "
						    + System.currentTimeMillis());
					}
					if (getPipRetrieval() == null && external.size() > 0) {
						try {
			...
			String requestString = JAXBUtility.marshalToString(RequestType.class,
			    requestType, "Request", JAXBUtility.SCHEMA);
	...
	/**
	 * From the list of attributes required by the actual policy, extract the ones
	 * related to external UCS. At first we try to see if the attribute is managed
	 * by one of the PIPs local to the ContextHandler,<b>NEW!!</b> then, if we
	 * cannot find the PIP among the local ones, we check to see if the attribute
	 * is already in the request, otherwise the attribute has to be retrieved from
	 * a remote PIP.
	 * 
	 * @param attributes
	 *          the list of attributes
	 ...
	 */
	private List<Attribute> extractExternal(List<Attribute> attributes,
	    RequestType request) {
			...
			if (found != true) {
				for (AttributesType attributeType : request.getAttributes()) {
					for (AttributeType att : attributeType.getAttribute()) {
						if (attributes.get(i).getAttributeId()
						    .equals(att.getAttributeId())) {
							found = true;
							break;
						}
					}
	...
	private String retrievePolicy(TryAccessMessage tryAccess) {
		...
		if (policy == null) {
			String policyString = getPapInterface()
			    .retrievePolicy(tryAccess.getPolicyId());
	 ...
	 *          the original request, not the fat one because, whenever we need to
	 *          re-evaluate the request we will retrieval from the various PIPs a
	 *          fresh value
	 * @param status
	 *          status of the request
	 * @param pepUri
	 *          the URI of the PEP in order to allow communication between the PEP
	 *          and the context handler
	 * @param policyHelper
	 *          object representing the policy to be used in the various
	 *          evaluations the subject id
	 */
	private void insertInSessionManager(String sessionId, String uxacmlPol,
	    String request, final String status, String pepUri,
	    PolicyHelper policyHelper, String ip) {
		try {
			RequestType requestType = JAXBUtility.unmarshalToObject(RequestType.class,
			    request);
			
			// retrieve the id of ongoing attributes
			List<Attribute> onGoingAttributes = policyHelper
			    .getAttributesForCondition(STARTACCESS_POLICY);
			List<String> onGoingAttributesForSubject = getAttributesForCategory(
			    onGoingAttributes, Category.SUBJECT);
			List<String> onGoingAttributesForResource = getAttributesForCategory(
			    onGoingAttributes, Category.RESOURCE);
			List<String> onGoingAttributesForAction = getAttributesForCategory(
			    onGoingAttributes, Category.ACTION);
			List<String> onGoingAttributesForEnvironment = getAttributesForCategory(
			    onGoingAttributes, Category.ENVIRONMENT);
			...
			if (!getSessionManagerInterface().createEntry(sessionId, uxacmlPol,
			    request, onGoingAttributesForSubject, onGoingAttributesForResource,
			    onGoingAttributesForAction, onGoingAttributesForEnvironment, status,
			    pepUri, ip, subjectName, resourceName, actionName)) {
				LOGGER.log(Level.SEVERE,
				    "[Context Handler] TryAccess: some error occurred, session "
				        + sessionId + " has not been stored correctly");
			}
		} catch (Exception e) {
			e.printStackTrace();
			return;
	 ...
	 */
	private List<String> getAttributesForCategory(
	    List<Attribute> onGoingAttributes, Category category) {
	 ...
	 * The following actions are performed:
	 * <ol>
	 * <li>the session that should be reevaluated is retrieved through the session
	 * manager</li>
	...
	public void startAccess(Message message) throws Exception {
		...
		if (!isInitialized() || message == null
			...
			LOGGER.log(Level.SEVERE, "INVALID startAccess " + isInitialized() + "\t"
			    + (message instanceof StartAccessMessage));
			return;
		}
		// END parameter checking
		...
		System.out
		    .println("[TIME] startaccess begins at " + System.currentTimeMillis());
		...
		PolicyHelper policyHelper = PolicyHelper
		    .buildPolicyHelper(sessionToReevaluate.getPolicySet());
		...
		// session id
		if (sessionToReevaluate == null
		    || (!sessionToReevaluate.getStatus().equals(TRY_STATUS))) {
			// no request to reevaluate(some problem occurred during request and
			// policy retrieving)
			LOGGER.log(Level.WARNING,
			    "[Context Handler] Startaccess: tryaccess must be performed yet for session "
			        + sessionId);
		...
		// make the request complete before reevaluation
		String requestFull = makeRequestFull(request,
		    policyHelper.getAttributesForCondition(STARTACCESS_POLICY),
		    STATUS.STARTACCESS, true);
		
		// perform the evaluation
		PDPResponse pdpEvaluation = (PDPResponse) getPdpInterface().evaluate(
		    requestFull,
		    policyHelper.getConditionForEvaluation(STARTACCESS_POLICY));
		
		System.out.println(
		    "[TIME] startaccess evaluation ends at " + System.currentTimeMillis());
		...
		if (pdpEvaluation.getResponse().equalsIgnoreCase("Permit")) {
			...
			Boolean bool = getSessionManagerInterface().updateEntry(sessionId,
			    START_STATUS);
			
			if (bool == false) {
				LOGGER.log(Level.INFO, "[Context Handler] Startaccess: session "
				    + sessionId + " status not updated");
			}
			System.out.println(
			    "[TIME] PERMIT startaccess ends at " + System.currentTimeMillis());
			response.setStatus(pdpEvaluation.getResponse());
		}
			...
			// access must be revoked and the related entry from the db deleted, after
			// performing unsubscribe operation for the on going attributes
			if (revoke(sessionToReevaluate, attributesIP)) {
				LOGGER.log(Level.INFO,
				    "[Context Handler] Startaccess: access is revoked for session with ID: "
				        + sessionId);
	 ...
	 */
	private synchronized boolean revoke(SessionInterface session,
		...
		if (!otherSessions) {
			...
			// LOGGER.info(
			// "[Context Handler] Involved attributes should be unsubscribed. Every
	 ...
	 * <p>
	 * The first step is to retrieve the list of ongoing attributes, then we have
	 * to unsubscribe all those attributes that are not needed anymore.
	 * </p>
	 * 
	 * @param sessionId
	 ...
	 * @return true if threre are attributes to unsubscribe, false otherwise <br>
	 * 
	 */
	private boolean attributesToUnsubscribe(String sessionId,
		...
		Collection<OnGoingAttribute> onGoingAttributes = getSessionManagerInterface()
		    .getOnGoingAttributes(sessionId);
		...
		if (onGoingAttributes != null && onGoingAttributes.size() > 0) {
			...
			for (OnGoingAttribute attribute : onGoingAttributes) {
				if (attribute.getSubjectName() != null
				    && !attribute.getSubjectName().equals("null")) {
					onGoingAttributesForSubject.add(attribute);
					subjectName = attribute.getSubjectName();
				} else if (attribute.getResourceName() != null
				    && !attribute.getResourceName().equals("null")) {
					onGoingAttributesForResource.add(attribute);
					resourceName = attribute.getResourceName();
				} else if (attribute.getActionName() != null
				    && !attribute.getActionName().equals("null")) {
		...
		if (onGoingAttributes != null && onGoingAttributes.size() > 0) {
			...
			for (OnGoingAttribute attribute : onGoingAttributesForResource) {
				...
				// retrieve all the active sessions which deal with the considered on
				// going attribute
				List<SessionInterface> tempList = getSessionManagerInterface()
				    .getSessionsForResourceAttributes(resourceName,
				        attribute.getAttributeId());
				// check if there are not any active sessions which deal with the
				// attribute
			...
			for (OnGoingAttribute attribute : onGoingAttributesForSubject) {
				...
				// going attribute
				List<SessionInterface> tempList = getSessionManagerInterface()
				    .getSessionsForSubjectAttributes(subjectName,
				        attribute.getAttributeId());
				// check if there are not any active sessions which deal with the
				// attribute
				if (tempList == null || tempList.isEmpty() || tempList.size() == 1) {
			...
			for (OnGoingAttribute attribute : onGoingAttributesForAction) {
				List<SessionInterface> tempList = getSessionManagerInterface()
				    .getSessionsForActionAttributes(actionName,
				        attribute.getAttributeId());
				if (tempList == null || tempList.isEmpty() || tempList.size() == 1) {
					otherSessions = false;
					Attribute tmpAttribute = new Attribute();
	...
	public void endAccess(Message message) {
		...
		try {
			...
			System.out
			    .println("[TIME] endaccess begins at " + System.currentTimeMillis());
			
			// LOGGER.log(Level.INFO,
			// "[Context Handler] Endaccess is received for session ID: " + sId);
			
			// check if an entry actually exists in db
			SessionInterface sessionToReevaluate = getSessionManagerInterface()
			    .getSessionForId(endAccessMessage.getSessionId());
			if (sessionToReevaluate == null
			    || (!sessionToReevaluate.getStatus().equals(START_STATUS)
				...
				throw new WrongOrderException(
				    "[Context Handler] Endaccess: a tryaccess must be performed yet for session "
				        + sessionId
				        + ", or the related endaccess has already been executed");
			}
			...
			System.out.println(
			    "[TIME] endaccess scheduler starts at " + System.currentTimeMillis());
			List<Attribute> attributes = policyHelper
			...
			// make the request complete before reevaluation
			String requestFull = makeRequestFull(request,
			    policyHelper.getAttributesForCondition(ENDACCESS_POLICY),
			...
			System.out.println(
			    "[TIME] EndAccess evaluation ends at " + System.currentTimeMillis());
			...
			EndAccessResponse response = new EndAccessResponse(
			    endAccessMessage.getDestination(), endAccessMessage.getSource(),
			    message.getID());
			...
			if (revoke(sessionToReevaluate, attributesIP)) {
				// LOGGER.log(Level.INFO, "[Context Handler] Endaccess: Session with ID
				// "
				// + sId + " is terminated");
				System.out.println("[TIME] endaccess evaluation with revoke ends at "
				    + System.currentTimeMillis());
	...
	public void attributeChanged(Message message) {
		System.out
		    .println("Attribute changed received " + System.currentTimeMillis());
		// BEGIN parameter checking
	 ...
	 * 
	 * <p>
	 * The purpose of this function is to trigger the PIP in order to retrieve the
	 * value and then return this value to the caller
	 * </p>
	 */
	@Override
	public Message messageForPIP(Message message) {
		...
		try {
			...
			for (Attribute attribute : messagePipCh.getAttributes()) {
				for (PIPCHInterface pip : getPipList()) {
					// once you find the pip that manages that attribute, perform the
					// right action
					LinkedList<String> searchList = new LinkedList<>(
					    pip.getAttributeIds());
					if (searchList.contains(attribute.getAttributeId())) {
						...
						switch (messagePipCh.getAction()) {
							case RETRIEVE:
								attribute.setValue(
								    pip.getAttributesCharacteristics()
								        .get(attribute.getAttributeId()).getAttributeDataType(),
								    pip.retrieve(attribute));
								// System.out.println("Attribute" + new
								// Gson().toJson(attribute));
								break;
							case SUBSCRIBE:
								attribute.setValue(attribute.getAttributeDataType(),
								    pip.subscribe(attribute));
								break;
							default:
								break;
						}
			...
			switch (messagePipCh.getAction()) {
				case UNSUBSCRIBE:
					...
					MessagePipCh response = new MessagePipCh(getIp(),
					    messagePipCh.getSource());
	 ...
	 * subject) obviously check if the additional information stored in the policy
	 * is the same.</li>
	 * <li>For each session: ask the scheduler if it has to be evaluated locally
	 * or by a remote site, in the latter send the request with the values of the
	 * local attributes to the remote site.</li>
	 * </ol>
	 * <br>
	 * </p>
	 * <p>
	 * <b>Implemented behavior: </b> <br>
	 * This thread waits on the list named attributesChanged for a message coming
	 * from a PIP. When this happens the reevaluation process described above can
	 * start. Obviously it has to check if the considered session is already being
	 * evaluated, for eample the PEP may ask to terminate a session and in the
	 * meanwhile an attribute has changed, but the session has to be evaluated
	 * only once.
	 * </p>
	 * 
	 * @author antonio
	...
	final private class AttributeMonitor implements Runnable {
			...
			while (continueMonitor) {
				try {
					...
					if (!manageChanges(attributes,
					    !message.getDestination().equals(PART.CH.toString()))) {
		 ...
		 * additional informations] from the json object passed as parameter</li>
		 * <li>From these informations extract the list of interested sessions from
		 * the sessionmanager</li>
		 * <li>Divide the remote sessions, i.e., the sessions that belong to another
		 * context handler to the one this context handler is in charge of
		 * reevaluating</li>
		 * <ul>
		 ...
		 * 
		 * @param jsonObject
		 *          the jsonObject inserted by the PIP which attribute has changed
		...
		private boolean reevaluateSessions(Attribute attribute, boolean isRemote) {
			...
			try {
				...
				if (isRemote) {
					...
					HashMap<String, ArrayList<String>> remote = removeRemoteSessions(
					    interestedSessions);
		...
		/**
		 * Adds the attribute that has just changed to the request, in this way, if
		 * the session will be reevaluated remotely, it won't be necessary to
		 ...
		 */
		@Deprecated
		private void addAttributeToRequest(Attribute attribute,
		    List<SessionInterface> interestedSessions) throws JAXBException {
			for (SessionInterface session : interestedSessions) {
				String request = session.getOriginalRequest();
				RequestType requestType = JAXBUtility
				    .unmarshalToObject(RequestType.class, request);
				requestType.addAttribute(attribute.getCategory().toString(),
				    ...
				    attribute.getAttributeId(), attribute
				        .getAttributeValues(attribute.getAttributeDataType()).get(0));
				session.setRequest(JAXBUtility.marshalToString(RequestType.class,
				    requestType, "Request", JAXBUtility.SCHEMA));
			}
		}
		 ...
		 * informations: the attributeId and the eventual additional informations in
		 * order to retrieve all the sessions related to that particular attributeid
		 * with that particular additionalInformations. We need also the additional
		 * informations in order to perform a prefiltering, in fact it may happen,
		 * for example, that during time the role of a person changes, but we don't
		 * want to reevaluate all the sessions that use the attribute role, but only
		 * those sessions that are interested in that aprticular person.
		 * 
		 * @param attributeRetrieval
		 *          the attribute retrieval oject that represents the attribute that
		 *          has changed
		 * @return the list of sessions interested
		 */
		private List<SessionInterface> retrieveSessions(
		    Attribute attributeRetrieval) {
			boolean valid = false;
			// System.out
			// .println("AttributeRetrieval " + attributeRetrieval.getAttributeId()
			...
			if (attributeRetrieval.getCategory() == Category.RESOURCE) {
				...
				sessions = getSessionManagerInterface()
				    .getSessionsForResourceAttributes(
				        attributeRetrieval.getAdditionalInformations(),
			...
			if (attributeRetrieval.getCategory() == Category.ENVIRONMENT) {
				valid = true;
				sessions = getSessionManagerInterface()
				    .getSessionsForEnvironmentAttributes(
				        attributeRetrieval.getAttributeId());
			}
			if (valid) {
				return sessions;
		 ...
		 *         remote sessions, with the corresponding IPs in charge of handling
		 *         the reevaluation
		 */
		private HashMap<String, ArrayList<String>> removeRemoteSessions(
		    List<SessionInterface> interestedSessions) {
			...
			for (int i = 0; i < interestedSessions.size();) {
				 ...
				 */
				PolicyHelper policyHelper = PolicyHelper
				    .buildPolicyHelper(session.getPolicySet());
		...
		/**
		 * Notifies the remote sessions interested by the remote attribute that that
		 * attribute has changed, hence it is necessary for them to re-evaluate the
		 * sessions. As parameter it will be passed only the number of the sessions
		 * related to the attribute that has changed
		 * 
		 * 
		 * @param remote
		 ...
		 * changed.
		 * <p>
		 * Basically from each session it retrieves the original request and then
		 * asks to the PIPs to fill it with their attributes. Once the request is
		 * completed then it evaluates it in order to see which is the answer of the
		 * PDP.
		 * </p>
		 * After reevaluation of "local" sessions eventually notify the PEP of the
		 * changing happened, basically if the Request is not compliant with the
		 * policy anymore.
		 * 
		 * @param session
		 ...
		 * 
		 */
		private String reevaluateSession(SessionInterface session,
		    Attribute attribute) {
			try {
				System.out.println(
				    "[TIME] reevaluation begins at " + System.currentTimeMillis());
				...
				List<Attribute> attributes = policyHelper
				    .getAttributesForCondition(STARTACCESS_POLICY);
				System.out.println("[TIME] reevaluation scheduler starts at "
				    + System.currentTimeMillis());
				HashMap<String, Integer> attributesIP = retrieveAttributesIp(
				    attributes);
				//String ip = scheduler.getIp(attributesIP);
				/*if (!ip.equals(getIp()) && !ip.equals("localhost")
					...
					reevaluationMessage.setSession((Session) session);
					System.out.println(
					    "[TIME] reevaluation starts at " + System.currentTimeMillis());
	...
	public synchronized void reevaluate(Message message) {
		...
		PolicyHelper policyHelper = PolicyHelper
		    .buildPolicyHelper(reevaluationMessage.getSession().getPolicySet());
		// make the request complete before reevaluation
		String requestFull = makeRequestFull(request,
		    policyHelper.getAttributesForCondition(STARTACCESS_POLICY),
		    STATUS.STARTACCESS, true);
		...
		getObligationManager().translateObligations(pdpEvaluation,
		    reevaluationMessage.getSession().getId(), START_STATUS);
		
		// UXACMLPolicySet policySet = evaluate(request, uxacmlPol,
		// STARTACCESS_POLICY);
		
		System.out.println("[TIME] decision " + pdpEvaluation.getResponse()
		...
		System.out
		    .println("DESTINATION: " + destination + "\t" + session.getStatus());
		ReevaluationResponse chPepMessage = new ReevaluationResponse(getIp(),
		    destination);
		...
		getSessionManagerInterface().stopSession(session);
		if ((session.getStatus().equals(START_STATUS)
		    || session.getStatus().equals(TRY_STATUS))
		    && pdpEvaluation.getResponse().contains("Deny")) {
			LOGGER.log(Level.INFO,
			    "[TIME] Sending revoke " + System.currentTimeMillis());
		...
		if (session.getStatus().equals(REVOKE_STATUS)
		    && pdpEvaluation.getResponse().contains("Permit")) {
			LOGGER.log(Level.INFO,
			    "[TIME] Sending resume " + System.currentTimeMillis());
